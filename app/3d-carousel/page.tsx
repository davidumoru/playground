...
export default function ThreeDPhotoCarousel() {
  ...
  const cylinderWidth = isSmallScreen
    ? CAROUSEL_CONFIG.cylinderWidth.mobile
    : CAROUSEL_CONFIG.cylinderWidth.desktop;
  ...
  const handleDrag = useCallback(
    (_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      const newRotation =
        rotation.get() + info.delta.x * CAROUSEL_CONFIG.dragSensitivity;
  ...
  const handleDragEnd = useCallback(
    (_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      const momentum =
        Math.abs(info.velocity.x) > 100 ? info.velocity.x * 0.02 : 0;
  ...
  const handleWheel = useCallback(
    (event: React.WheelEvent) => {
      ...
      const targetRotation =
        rotation.get() + normalizedDelta * CAROUSEL_CONFIG.scrollSensitivity;
