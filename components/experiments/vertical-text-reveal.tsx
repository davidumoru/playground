...
function splitText(text: string) {
  return text.split("").map((char, i) => (
    <span className="letter" key={i} style={{ position: "relative", display: "inline-block" }}>
      {char === " " ? "\u00A0" : char}
    </span>
  ))
}
...
export default function VerticalTextReveal() {
  ...
  useEffect(() => {
    ...
    if (defaultNameRef.current) {
      const letters = Array.from(defaultNameRef.current.querySelectorAll(".letter"))
      gsap.set(letters, { y: "100%" })
    }
    ...
    if (!isMobile) {
      ...
      if (containerRef.current && defaultNameRef.current) {
        ...
        const defaultLetters = Array.from(defaultNameRef.current.querySelectorAll(".letter"))
      ...
      imagesRef.current.forEach((img, index) => {
        ...
        const onClick = () => {
          ...
          if (activeIndex !== null && namesRef.current[activeIndex]) {
            const prevLetters = Array.from(namesRef.current[activeIndex]!.querySelectorAll(".letter"))
  ...
  return (
    <div className="font-[family-name:var(--font-barlow-condensed)]">
      <section
        style={{
          position: "relative",
          width: "100vw",
